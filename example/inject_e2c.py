...
@app.command()
def inject_e2c(
    loader_exe_path: str, exe_src_path: str, cache_dst_path: str, exe_dst_path: str
) -> None:
    """
    inject Arm64 executable into cache file
    """
    for path in (loader_exe_path, exe_src_path, cache_dst_path, exe_dst_path):
    ...
    cache_dst = XtaCache(cache_dst_path, ["-w"])
    ...
    if (
        loader_entry_export := loader_exe_surf.find_export_loose("RunFromMemoryARM64")
    ) is None:
    ...
    print("load ARM64 exe")
    with open(exe_src_path, "rb") as fin:
        exe_payload = list(fin.read())
